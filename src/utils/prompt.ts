...
export async function promptTranslator() {
    const source_choices = Object.entries(Sources).map(([key, _]) => {
        return { name: language_choices[key], value: key, short: key };
    });
    ...
    }]).then((answers: any) => global.source = answers.source);
}
...
function getLanguageChoices(): { from_choices: LanguageCodes; to_choices: LanguageCodes; } {

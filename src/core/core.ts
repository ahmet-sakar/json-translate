...
export async function getFile(objectPath: string) {
    ...
    await fs.readFile(objectPath, 'utf8').then(data => json_file = data).catch(_ => json_file = undefined);
    return json_file;
}
...
export async function saveFilePublic(path: string, data: any) {
    await fs.writeFile(path, JSON.stringify(data), 'utf8').then(_ => {}).catch(_ => error(messages.file.cannot_save_file));
}

export function safeValueTransition(value: string) {
    const value_safety: ValueSafety = valueIsSafe(value);
    if (value_safety.is_safe === true) return value;
...
function valueIsSafe(value: string): ValueSafety {
    let result: ValueSafety = { is_safe: true, type: undefined };
    if (value === undefined) {
...
interface ValueSafety { is_safe: boolean; type: nonSafeTypes | undefined; }
enum nonSafeTypes { 'long', 'undefined', 'null', 'empty' }

export function map(str: string): {
    word: string;
    ...
    let { map: single_brackets_map, word: ignored_word } = mapBySingleBracket(initial_ignored_word);
    return {
        word: ignored_word,
        double_brackets_map,
...
export function unMap(str: string, double_brackets_map: object, single_brackets_map: object): string {
    let word = unmapBySingleBracket(str, single_brackets_map);
    word = unmapByDoubleBracket(word, double_brackets_map);
    word = urlDecoder(word); // decode urls if exists in the str
    return word;
}

function mapBySingleBracket(str: string): { word: string; map: { [key: string]: string } } {
...
function mapByDoubleBracket(str: string): { word: string; map: { [key: string]: string } } {
    return mapIgnoredValues(str, '{{', '}}', '{', '}');
}
...
function mapIgnoredValues(str: string, start: string, end: string, replaced_start: string, replaced_end: string): { word: string; map: { [key: string]: string } } {
    let counter = 0;
    ...
    let new_str = str.replace(regex, function(word) {
        ...
        const key = counter; // const key = "*".repeat(counter)
        map[`${key}`] = word;
...
function unmapIgnoredValues(str: string, map: object, start: string, end: string, replaced_start: string, replaced_end: string): string {
...
function urlEncoder(text: string): string { // URL detector & encode AND decoder
...
function urlDecoder(text: string): string { // url finder regex => {url}
    const regex = /{(?:(?:https?|ftp|file):\/\/|www\.|ftp\.)(?:\([-A-Z0-9+&@#\/%=~_|$?!;:,.]*\)|[-A-Z0-9+&@#\/%=~_|$?!;:,.])*(?:\([-A-Z0-9+&@#\/%=~_|$?!;:,.]*\)|[A-Z0-9+&@#\/%=~_|$])}/gim;
    let new_text = text.replace(regex, function(url) {

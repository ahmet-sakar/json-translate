...
export async function fileTranslator(objectPath: string, from: LanguageCode, to: LanguageCode | LanguageCodes) {
    ...
    if (Array.isArray(new_json_obj) === true && Array.isArray(to) === true) {
        (new_json_obj as Array<translatedObject>).forEach( // multiple file saving
            async (element, index) => {
...
export async function getFileFromPath(objectPath: string): Promise<{ json_obj: any; objectPath: string }> {

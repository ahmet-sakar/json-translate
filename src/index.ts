...
export async function translateWord(word: string, from: LanguageCode, to: LanguageCode) {
    return await plaintranslate(word, from, to);
}

export async function translateObject(object: translatedObject, from: LanguageCode, to: LanguageCode | LanguageCodes): Promise<translatedObject | translatedObject[]> {
    let hard_copy = JSON.parse(JSON.stringify(object));
    return objectTranslator(hard_copy, from, to);
}

export async function translateFile(objectPath: string, from: LanguageCode, to: LanguageCode | LanguageCodes) {
...
global.source = Sources.GoogleTranslate; // default
export const translatorsNames = Object.values(Sources).map(s => s.split('Translate')[0].toLowerCase());
export interface translatedObject { [key: string]: any; } // TYPES
export type LanguageCode = string;
export type LanguageCodes = LanguageCode[];
export function getLanguages() {
    if (global.source === Sources.LibreTranslate) return LibreTranslateLanguages;
    else if (global.source === Sources.ArgosTranslate) return ArgosTranslateLanguages;
    else if (global.source === Sources.BingTranslate) return BingTranslateLanguages;
